...
export const setting = {
	add: function (setting: { id: IDBValidKey; [x: string]: any }) {
		return new Promise<any>((resolve, reject) => {
			db.open((db) => {
				const settingObjectStore = db.transaction('setting', 'readwrite').objectStore('setting');
	...
	get: function (id: IDBValidKey) {
		return new Promise<any>((resolve, reject) => {
			db.open((db) => {
				const settingObjectStore = db.transaction('setting').objectStore('setting');
	...
	getAll: function () {
		return new Promise<{ [x: string]: any }>((resolve, reject) => {
			db.open((db) => {
				const cursorRequest = db.transaction('setting').objectStore('setting').openCursor();
				...
				cursorRequest.onsuccess = () => {
					...
					if (cursor) {
						const { id, ...setting } = cursorRequest.result.value;
						settings[id] = setting;
						cursor.continue();
					} else {
