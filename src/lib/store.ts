...
export let state = writable<State>(
	(browser && localStorage.getItem('card') && JSON.parse(localStorage.getItem('card')!)) || {
		cards: [],
		csvs: [],
		drag: {
			isInProgress: false,
...
state.subscribe((value) => {
	const { template, print, csvs } = value;
...
export const activeSidebarMenu = writable<'card' | 'color' | 'text' | 'image' | 'print' | null>(
	'card'
);
export const activeTabs = writable<
	Map<
		IDBValidKey,
		{ id: IDBValidKey; itemId: IDBValidKey; title: string; type: 'template' | 'palette' }
	>
>(new Map());
...
export const activeElement = writable<IDBValidKey | null>(undefined);
