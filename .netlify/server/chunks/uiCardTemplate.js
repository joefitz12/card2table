...
class UICardTemplate extends CardTemplate {
	...
	handleDrop;
	handleDragover;
	constructor({ id, unit, height, width, title, color, backgroundColor, padding, border }) {
		super();
		this.id = id;
		this.unit = unit;
		this.height = height;
		...
		this.handleDrop = (e) => {
			...
			textElements.update(($textElements) => {
				...
				[...$textElements.values()].findIndex((element) => element.template.id == textElementId);
				if (!textElement) {
					return $textElements;
				}
				const cardOffset = {
				...
				if (
					!Math.abs(cardOffset.offsetX - get_store_value(state).drag.offsetX) ||
					!Math.abs(cardOffset.offsetY - get_store_value(state).drag.offsetY)
				) {
					return $textElements;
				}
...
export { CardTemplate as C, UICardTemplate as U };
