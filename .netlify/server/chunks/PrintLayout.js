...
const PrintLayout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<div class="flex row print-container svelte-ng6bob">${
		$template
			? `<div class="${[
					...
					.trim()}" style="${
					'--column-gap: ' +
					escape($print.columnGap + ($template.unit || 'in'), true) +
					'; --row-gap: ' +
					escape(rowGap + ($template.unit || 'in'), true) +
					'; --card-width: ' +
					escape(($template.width || 2.44) + ($template.unit || 'in'), true) +
					'; --page-height: ' +
					escape($print.height + ($template.unit || 'in'), true) +
					'; --page-width: ' +
					escape($print.width + ($template.unit || 'in'), true) +
					'; --rows-per-page: ' +
					escape(rowsPerPage, true) +
					';'
			  }">${
					$print.width < $template.width || $print.height < $template.height
						? `<div class="flex column align-center" data-svelte-h="svelte-7m0czx"><span>Page too small =(</span></div>`
						: ``
			  } ${
					$csvs.length && !($print.width < $template.width || $print.height < $template.height)
						? `${each([...$cards.values()], (card) => {
								return `<div class="card-template svelte-ng6bob" style="${
									'--card-height: ' +
									escape(($template.height || 3.43) + ($template.unit || 'in'), true) +
									'; --card-width: ' +
									escape(($template.width || 2.44) + ($template.unit || 'in'), true) +
									'; --card-border-color: ' +
									escape($template.border.color, true) +
									'; --card-border-top-width: ' +
									escape(($template.border.width.top || 0) + ($template.unit || 'in'), true) +
									'; --card-border-right-width: ' +
									escape(($template.border.width.right || 0) + ($template.unit || 'in'), true) +
									'; --card-border-bottom-width: ' +
									escape(($template.border.width.bottom || 0) + ($template.unit || 'in'), true) +
									'; --card-border-left-width: ' +
									escape(($template.border.width.left || 0) + ($template.unit || 'in'), true) +
									'; --card-border-top-left-radius: ' +
									escape(($template.border.radius.topLeft || 0) + ($template.unit || 'in'), true) +
									'; --card-border-top-right-radius: ' +
									escape(($template.border.radius.topRight || 0) + ($template.unit || 'in'), true) +
									'; --card-border-bottom-right-radius: ' +
									escape(
										($template.border.radius.bottomRight || 0) + ($template.unit || 'in'),
										true
									) +
									'; --card-border-bottom-left-radius: ' +
									escape(
										($template.border.radius.bottomLeft || 0) + ($template.unit || 'in'),
										true
									) +
									'; --card-background-color: ' +
									escape($template.backgroundColor, true) +
									'; --card-top-padding: ' +
									escape(($template.padding.top || 0) + ($template.unit || 'in'), true) +
									'; --card-right-padding: ' +
									escape(($template.padding.right || 0) + ($template.unit || 'in'), true) +
									'; --card-bottom-padding: ' +
									escape(($template.padding.bottom || 0) + ($template.unit || 'in'), true) +
									'; --card-left-padding: ' +
									escape(($template.padding.left || 0) + ($template.unit || 'in'), true) +
									';'
								}"><div class="overlay svelte-ng6bob"></div> ${each(
									...
									([key, textElement]) => {
										return `<div class="${[
											...
											.trim()}" style="${
											'--color: ' +
											escape(textElement.color, true) +
											'; --font-size: ' +
											escape((textElement.fontSize || 0.22) + ($template.unit || 'in'), true) +
											'; --transform-left: ' +
											escape(
												(textElement.leftTransform || 0) / $template.relativeUnit +
													($template.unit || 'in'),
												true
											) +
											'; --transform-top: ' +
											escape(
												(textElement.topTransform || 0) / $template.relativeUnit +
													($template.unit || 'in'),
												true
											) +
											'; --font-weight: ' +
											escape(textElement.fontWeight, true) +
											'; --font-style: ' +
											escape(textElement.fontStyle || 'normal', true) +
											'; --text-decoration: ' +
											escape(textElement.textDecoration || 'normal', true) +
											'; --padding-top: ' +
											escape((textElement.padding.top || 0) + ($template.unit || 'in'), true) +
											'; --padding-right: ' +
											escape((textElement.padding.right || 0) + ($template.unit || 'in'), true) +
											'; --padding-bottom: ' +
											escape((textElement.padding.bottom || 0) + ($template.unit || 'in'), true) +
											'; --padding-left: ' +
											escape((textElement.padding.left || 0) + ($template.unit || 'in'), true) +
											'; --border-top-width: ' +
											escape((textElement.border.width.top || 0) + ($template.unit || 'in'), true) +
											'; --border-right-width: ' +
											escape(
												(textElement.border.width.right || 0) + ($template.unit || 'in'),
												true
											) +
											'; --border-bottom-width: ' +
											escape(
												(textElement.border.width.bottom || 0) + ($template.unit || 'in'),
												true
											) +
											'; --border-left-width: ' +
											escape(
												(textElement.border.width.left || 0) + ($template.unit || 'in'),
												true
											) +
											'; --border-top-left-radius: ' +
											escape(
												(textElement.border.radius.topLeft || 0) + ($template.unit || 'in'),
												true
											) +
											'; --border-top-right-radius: ' +
											escape(
												(textElement.border.radius.topRight || 0) + ($template.unit || 'in'),
												true
											) +
											'; --border-bottom-right-radius: ' +
											escape(
												(textElement.border.radius.bottomRight || 0) + ($template.unit || 'in'),
												true
											) +
											'; --border-bottom-left-radius: ' +
											escape(
												(textElement.border.radius.bottomLeft || 0) + ($template.unit || 'in'),
												true
											) +
											';'
										}"><span class="text-element svelte-ng6bob">${escape(
											card[textElement.title.toLowerCase().split(' ').join('-')]
										)}</span> </div>`;
									}
								)} </div>`;
						  })}`
